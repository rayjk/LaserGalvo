...
fftw_complex SIGNAL[FFT_AVG][TOT_SAMPLES];
...
void get_stdin_audio(){
  ...
  for(int i=0; i<TOT_SAMPLES; ++i){
    SIGNAL[(int)i/(TOT_SAMPLES)][i%(TOT_SAMPLES)][REAL] = SHIFT_SIG[(ARR_END + i) % (TOT_SAMPLES)];
    //cout << SIGNAL[i][REAL] << ',';
  }
  //cout << endl;
}
...
void getFreqArr(double* freq_arr) {
  ...
  int lastIdx = 0;
  double magVal;
  fftw_complex result[FFT_AVG][TOT_SAMPLES];
  fftw_plan plan[FFT_AVG];
  for (int i=0; i<FFT_AVG; ++i){
    if(i%2 == 0){
      plan[i] = fftw_plan_dft_1d(TOT_SAMPLES,
                                        SIGNAL[i],
                                        result[i],
                                        FFTW_FORWARD,
                                        FFTW_ESTIMATE);
   ...
   // 	break;
    ...
    //wait for ready status
      do {
        ...
        lastLatency = (chrono::duration_cast<std::chrono::microseconds>(stopBuf - startBuf)).count();
      ...
      if (i%100 == 0){
        ...
        cout << "Last buffer latency (ms): " << (float)lastLatency / 1000. << endl;
