...
laser:  AudioSample libHeliosDacAPI.so $(HOBJS)
	g++ -o bin/LaserShow $(HOBJS) $(LLDFLAGS) $(PERF) LaserShow.cpp

laser-rpi:  AudioSample-rpi libHeliosDacAPI-rpi.so $(HOBJSR)
...
AudioSample-rpi:  AudioSample-rpi.cpp AudioBuffer-rpi.o
...
clean:
	rm bin/* *.o libHeliosDacAPI.so libHeliosDacAPI-rpi.so
